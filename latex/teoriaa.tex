...
\begin{esim}
	...
	\begin{displaymath}
		...
		\end{pmatrix}
	\end{displaymath}
	Tämä voidaan visualisoida seuraavanlaisesti:
...
\begin{maar}
	Kun $S$ on rajoittamaton, siirtymämatriisi on parasta ajatella kuvauksena $\pr: S\times S \rightarrow [0,1]$, joka kuvaa tilaparin $x,y\in S$ todennäköisyydeksi $p(x,y)$
\end{maar}

\begin{maar}
	Iistymätodennäköisyys $p^{(n)}_{ij}$ voidaan kirjoittaa \textit{siirtymätiheytenä} $p^{(n)}(x,y)$, jolle pätee
	\begin{equation}\label{siirt-tiheys}
		\int_S p(x,y)dy = 1 \hspace{1cm} \text{ja} \hspace{1cm} \int_S p^{(n)}(x,y)dy = 1, \forall n \geq 1
	\end{equation}
\end{maar}
...
\begin{proof}
	Yhtälön \ref{siirt-tiheys} mukaan $\int_S p(y,x)dx = 1$, joten
	\begin{equation}
